var MapsGoogle = function () {
    ...
    var mapMarker = function (lists) {
        var firstLat = -12.042 ;
        var firstLng = -77.028333 ;
        for (var key in lists) {
            firstLat = lists[key]['destination']['location']['lat'] ;
            firstLng = lists[key]['destination']['location']['lng'] ;
        }
        var map = new GMaps({
            div: '#gmap_marker',
            lat: firstLat,
        ...
        for (var key in lists) {
            ...
            map.addMarker({
                lat: firstLat,
                lng: firstLng,
                title: lists[key]['driverName'],
                icon: '../'+lists[key]['carIcon'],
                details: {
                    database_id: 42,
                    author: 'HPNeo'
                },
                infoWindow: {
                    content: '<span style="color:#000">Driver: '+lists[key]['driverName']+'</span>'+
                            '<br><span style="color:#000">Passenger: '+lists[key]['customerName']+'</span>'+
                            '<br><span style="color:#000">Car Type: '+lists[key]['carType']+'</span>'
                }
            });
        }
        map.setZoom(15);
